...
if [[ $IDEMPIERE_SOURCE_FOLDER == "/*" ]]; then
	...
	UX_IDEMPIERE_SOURCE_FOLDER="$DIR/$IDEMPIERE_SOURCE_FOLDER"
	IDEMPIERE_SOURCE_FOLDER=$(cygpath -w $UX_IDEMPIERE_SOURCE_FOLDER)
	
fi

cd $ECLIPSE
UX_DESTINATION=$(pwd)
DESTINATION=$(cygpath -w $UX_DESTINATION)
