...
if [ ! -f eclipse-jee-2023-06-R-win32-x86_64.zip ]; then
        echo
        echo "*** Download Eclipse ***"
        echo
        curl -OJL https://download.eclipse.org/technology/epp/downloads/release/2023-06/R/eclipse-jee-2023-06-R-win32-x86_64.zip
fi
if [ ! -d $ECLIPSE ]; then
        echo
        ...
        unzip eclipse-jee-2023-06-R-win32-x86_64.zip
        ECLIPSE=eclipse
fi

UX_JAVA_HOME=$(pwd)/eclipse/plugins/org.eclipse.justj.openjdk.hotspot.jre.full.win32.x86_64_17.0.7.v20230425-1502/jre
WIN_JAVA_HOME=$(cygpath -w $UX_JAVA_HOME)
...
if [ ! -f apache-groovy-binary-4.0.13.zip ]; then
	...
	echo
	curl -OJL https://archive.apache.org/dist/groovy/4.0.13/distribution/apache-groovy-binary-4.0.13.zip
...
WIN_RECENT_WORKSPACES=$(cygpath -w $PWD/idempiere)
RECENT_WORKSPACES=$(echo "$WIN_RECENT_WORKSPACES" | sed 's/\\/\\\\/g')
...
if [ ! -f $ECLIPSE/configuration/.settings/org.eclipse.ui.ide.prefs ]; then
	...
	echo "RECENT_WORKSPACES=$RECENT_WORKSPACES"  >> $IDE_PREFERENCE
